...
def generar_evaluaciones_historicas(datos, minimo_historial=50):
	...
	if datos is None or datos.empty or isinstance(minimo_historial, bool) or not isinstance(minimo_historial, int) or minimo_historial <= 0:
		return []
...
def calcular_resultados_futuros(datos, evaluaciones, horizontes=(1, 5, 10)):
	...
	for evaluacion in evaluaciones:
		...
		if pd.isna(precio_base) or not math.isfinite(precio_base) or precio_base <= 0:
			precio_base = None
		...
		for horizonte in horizontes_validos:
			...
			if pd.isna(precio_futuro) or not math.isfinite(precio_futuro) or precio_futuro <= 0:
				retornos_futuros[horizonte] = None
				continue
...
def calcular_estadisticas_por_favorables(resultados, horizontes=(1, 5, 10)):
	...
	for resultado in resultados:
		...
		if isinstance(favorables, bool) or not isinstance(favorables, (int, float)):
			continue
		if not math.isfinite(favorables):
			continue
